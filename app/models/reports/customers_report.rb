class CustomersReport < ReportBase
  ...
  def results
    sql = %q{
      SELECT A.year
           , B.month
           , SUM(CASE WHEN stripe_customers.id IS NULL THEN 0 ELSE 1 END) AS amount
           , goals.value AS goal
        ...
        LEFT JOIN stripe_customers ON stripe_customers.organization_id = :organization_id
                              AND stripe_customers.created_at BETWEEN (to_date(to_char(B.month, '00') || to_char(A.year,'9999'), 'MMYYYY'))::DATE
                                                                  AND (to_date(to_char(B.month, '00') || to_char(A.year,'9999'), 'MMYYYY') + INTERVAL '1 MONTH - 1 day')::DATE
        LEFT JOIN goals ON goals.organization_id = :organization_id
                       AND goals.year = A.year
                       AND goals.month = B.month
                       AND goals.metric = 'customers_count'
        GROUP BY A.year
               , B.month
               , goals.value
               , goals.id
